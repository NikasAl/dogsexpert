 ...
 */
package ru.electronikas.dogsexpert.neural.image;
 ...
 * @author jheaton
 */
public class PixmapMLDataSet extends BasicMLDataSet {
	 ...
	 *            The low value to normalize to.
	 */
	public PixmapMLDataSet(final Downsample theDownsampler,
	...
	public final void downsample(final int theHeight, final int theWidth) {
		...
		for (final MLDataPair pair : this) {
			if (!(pair.getInput() instanceof PixmapMLData)) {
				throw new NeuralNetworkError(
						"Invalid class type found in ImageNeuralDataSet, only "
								+ "ImageNeuralData items are allowed.");
			}

			final PixmapMLData input = (PixmapMLData) pair.getInput();
